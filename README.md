...
| **额外码表** | 开启加载额外码表。**⚠️警告：大量加载码表会占用大量内存，可能会导致设备因内存不足而卡死。启用前请务必先备份完整 `koreader` 文件夹内容！** |
...
- 自定义码表：如果需要添加自定义码表，仅需将自制的码表文件（需确保格式正确）放到文件夹`lexicon`下，加载插件时会自动扫描该文件夹内的所有码表并出现在`额外码表`菜单中，在菜单中找到该码表启用即可。
...
| 文件名 | 菜单中文名 | 类型 |
|--------|-----------|------|
| `Classical Poetry.lua` | 古诗词词库 | 文件 |
| `Finance.lua` | 财经金融词库 | 文件 |
| `Ideological.lua` | 思政专业术语词库 | 文件 |
| `Idiom` | 成语俗语词库 | 文件夹 |
| `Legal.lua` | 法律词库 | 文件 |
| `Neologisms.lua` | 新词集锦词库 | 文件 |
| `Three-Character Idiom.lua` | 三字成语词库 | 文件 |
| `WittySaying.lua` | 歇后语词库 | 文件 |
